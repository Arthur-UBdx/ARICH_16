...
from pythonlib.data import REGEX_COMM, Instruction, Macro, Memory, Registers
...
class Assembler:
    def __init__(self, input_filename, output_filename: str = "output.lgsim"):
        self.instructions_dict = {
            i.mnemonic: i for i in Instruction.from_file(INSTRUCTIONS_LIST_FILE)
        }
    ...
    def new_file(filename: str, force=False) -> None:
        if not force and os.path.exists(filename):
            raise FileExistsError(
                f"File {filename} already exists, use '-f' to overwrite it"
            )
    ...
    def expand_replacements(self, line: str) -> str:
        for key, value in self.replacements.items():
            # line = re.sub(r"([ \|&\$\[\%\:])" + re.escape(key), r"\1" + value, line)
            # ça marche pas jsp pourquoi alors que en test ça marche donc on fait un replace de bourrin
    ...
    def parse_macro(self, line: str) -> None:
        ...
        match macro.keyword:
            case "stack":
                self.binary_code += [
                    0x101,
                    self.into_number(tokens[1]),
                    0x12,
                ]  # put the given value as immediate to the base pointer
                self.memory.stack_base_address = self.into_number(tokens[1])

            case "define":
                if tokens[1][0].isdigit():
                    raise MacroError(
                        f"Variable/replacement name {tokens[1]} cannot start with a digit"
                    )
            ...
            case "let":
                ...
                if re.match(r".+\[.+\]", tokens[1]):  # if trying to define an array
                    if re.search(r".+\[([0-9]+)\]", tokens[1]) is None:
                        raise MacroError(f"Invalid array size for {tokens[1]}")
                    size = int(re.search(r".+\[([0-9]+)\]", tokens[1]).group(1))
                    name = re.search(r"(.+)\[.+\]", tokens[1]).group(1)
                    if size == 0:
                    ...
                    for index, address in enumerate(addresses):
                        self.variables[f"{name}[{index}]"] = address
                else:
                    self.variables[tokens[1]] = self.memory.assign_address(
                        self.into_number(address) if address is not None else None
                    )
            ...
            case "deref":
                ...
                if tokens[1][-2:] == "[]":
                    ...
                    while f"{tokens[1][:-2]}[{i}]" in self.variables:
                        self.memory.used_addresses.remove(
                            self.variables[f"{tokens[1]}[{i}]"]
                        )
                        i += 1
                else:
            ...
            case "letreg":
                if tokens[1][0].isdigit():
                    raise MacroError(
                        f"Register name {tokens[1]} cannot start with a digit"
                    )
    ...
    def parse_line(self, line: str, debug=False) -> None:
        line = self.remove_comments(line).strip()
        line = self.expand_replacements(line)
        ...
        if len(tokens) - 1 < instruction.nb_operands:
            raise SyntaxError(
                f"Too few operands for instruction {instruction.mnemonic}"
            )
        if len(tokens) - 1 > instruction.nb_operands:
            raise SyntaxError(
                f"Too many operands for instruction {instruction.mnemonic}"
            )
        ...
        for index, operand in enumerate(
            tokens[1:]
        ):  # replace registers, variables and labels
            match operand[0]:
                case "%":
                ...
                case "&":
                    if (
                        index + 1 == instruction.output_operand
                    ):  # output_operand = 0 -> no output operand, 1 -> first operand etc...
                        raise SyntaxError(
                            "The output operand cannot be passed as immediate value"
                        )
                    if operand[1:] not in self.variables:
                        raise VariableError(
                            f"Variable {operand[1:]} is not (yet) defined"
                        )
                    operand = self.variables[operand[1:]]
                    opcode = opcode | 1 << (
                        8 + index
                    )  # set the immediate bit (8th for the first operand, 9th for the second)
        ...
        if VERBOSE:
            print(
                f"{f'{self.binary_code.__len__():03X}'}:\t{line} {(40-line.__len__())*' '}-> {f'{opcode:04X}' + ' ' + ' '.join([f'{o:04X}' for o in operands])}"
            )
        if debug:
            self.debug_log(
                f"{f'{self.binary_code.__len__():03X}'}:\t{line} {(40-line.__len__())*' '}-> {f'{opcode:04X}' + ' ' + ' '.join([f'{o:04X}' for o in operands])}"
    ...
    def assemble(self, debug=False) -> None:
        ...
        if VERBOSE:
            ...
            print(
                "\nFINAL BINARY CODE: "
                + " ".join([f"{code:04X}" for code in self.binary_code])
            )
        if debug:
            self.debug_log(f"{self.variables}")
            self.debug_log(
                "//\nFINAL BINARY CODE: "
                + " ".join([f"{code:04X}" for code in self.binary_code])
                + "\n//"
        ...
        self.output_file.insert_list(self.binary_code).to_file(
            self.output_filename, force=FORCE
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--output",
        type=str,
        help="Output file for the assembler",
        default="output.bin",
    )
    parser.add_argument(
        "-n",
        "--new",
        help="Creates a new .arich file with the default instructions",
        action="store_true",
    )
    parser.add_argument(
        "-f",
    ...
    parser.add_argument(
        "-v", "--verbose", help="Prints the binary code generated", action="store_true"
    )
    parser.add_argument(
        "-d",
        "--debug",
        help="Creates a debug file with helpful information",
        action="store_true",
    )
